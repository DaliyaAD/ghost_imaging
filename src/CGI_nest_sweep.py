...
def cumulative_values(phantom, pattern, arr_size, samp_rat_max, arr_seed=None):
    M_max = sampling_to_M(samp_rat_max, arr_size)
    full_stack = generate_pattern(
        pattern, M_max, arr_size=arr_size, arr_seed=arr_seed)
    bucket_all = compute_bucket_values(full_stack, phantom)

    weighted = full_stack * bucket_all[:, None, None]
    cm_weighted = np.cumsum(weighted, axis=0)
    cm_bucket = np.cumsum(bucket_all)
    cm_stack = np.cumsum(full_stack, axis=0)

    return cm_weighted, cm_bucket, cm_stack


def reconstruction(cm_weighted, cm_bucket, cm_stack, samp_rat, arr_size):
    M = sampling_to_M(samp_rat, arr_size)
    mean_weighted = cm_weighted[M - 1] / M
    mean_bucket = cm_bucket[M - 1] / M
    mean_stack = cm_stack[M - 1] / M
    return mean_weighted - mean_bucket * mean_stack, M
...
def main(CONFIG):
    ...
    sampling_ratios = CONFIG['sampling_ratios']   # e.g. [5, 10, 20, 50, 100]
    samp_rat_max = max(sampling_ratios)
    num_seeds = CONFIG['num_seeds']
    seed_int = CONFIG.get('seed_int', 10)
    patterns = CONFIG.get('patterns', ['binary', 'gaussian'])

    seeds = range(1, num_seeds, seed_int)
    results = []
    table = []

    for pattern, sd in itertools.product(patterns, seeds):
        cm_weighted, cm_bucket, cm_stack = cumulative_values(
            phantom, pattern, arr_size, samp_rat_max, arr_seed=sd)

        for samp_rat in sampling_ratios:
            start = time.perf_counter()
            recon, M = reconstruction(
                cm_weighted, cm_bucket, cm_stack, samp_rat, arr_size)
            ...
            results.append(row + (samp_rat,)+(runtime,))
            nmse, psnr, ssim = row[3], row[4], row[5]
            table.append([samp_rat, pattern, sd, nmse, psnr, ssim])

    headers = ["Phantom", "Pattern", "M",
               "NMSE", "PSNR", "SSIM", "Seed", "Array size", "Sampling ratio (%)", "Runtime (s)"]
    tab_headers = ["Sampling ratio (%)", "Pattern",
                   "Seed", "NMSE", "PSNR", "SSIM"]
    df = pd.DataFrame(results, columns=headers)
    df.to_csv(f"Results_{phantom_type}.csv", index=False)
...
CONFIG = {
    'arr_size': 32,
    'phantom': 'A',
    'sampling_ratios': [5, 10, 20, 50, 100],
    'num_seeds': 3,
    'seed_int': 1,
